	...
	private void Update ()
	{
		...
		if (gos.Length < enemyCount) {
			...
			if (timeSpawn >= timespawnmax) {
                GameObject enemyCreated = (GameObject) Instantiate (Objectman.gameObject,
                    transform.position +new Vector3 (Random.Range (-radian, radian), 
                    20, 
                    Random.Range (-radian, radian)),Quaternion.identity);
